import { MdOutlineLocalOffer } from "react-icons/md";
import { NavLink } from "react-router-dom";
import { FaStar } from "react-icons/fa";

function Hero() {
  return (
    <section className="relative bg-hero bg-center bg-no-repeat h-screen w-full">
      <div className="max_padd_container relative top-32 xs:top-52"> 
        {/* Headline */}
        <h1 className="h1 capitalize max-w-[32rem]">Shop the Latest Trends</h1>

        {/* Subheading */}
        <p className="text-gray-50 regular-16 mt-16 max-w-[33rem]">
          Discover high-quality clothing with styles to fit every occasion. Join
          our happy customers who trust us for fashion that lasts.
        </p>

        {/* Reviews and Rating Section */}
        <div className="flexStart items-center gap-x-4 my-10">
          <div className="regular-24 flexCenter gap-x-3">
            {/* 5 Stars */}
            {[...Array(5)].map((_, index) => (
              <FaStar key={index} />
            ))}
          </div>

          {/* Review Count */}
          <div className="bold-16 sm:bold-20">
            167k{" "}
            <span className="regular-16 sm:regular-20">Excellent Reviews</span>
          </div>
        </div>

        {/* Buttons */}
        <div className="flex gap-2 max-xs:flex-col">
          <NavLink to="/" className="btn_dark_rounded flexCenter">
            Shop Now
          </NavLink>
          <NavLink to="/" className="btn_dark_rounded flexCenter gap-x-2">
            <MdOutlineLocalOffer className="text-2xl" />
            Offers
          </NavLink>
        </div>
      </div>
    </section>
  );
}

export default Hero;
